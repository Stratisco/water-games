...
var custom = {
    ...
    'maxSpeed': 10,
    'firerate': 0,
    'despawn': (30*this.firerate)+100,
    'refresh': function() {
        this.acceleration = document.querySelector("#customAcc")
        this.maxSpeed = document.querySelector("#customSpeed")
        this.firerate = document.querySelector("#customFirerate")
    },
}

//var mainLoop = setInterval(custom.refresh(), 10)
...
Ship = function () {
  ...
  this.preMove = function (delta) {
    ...
    if (KEY_STATUS.space) {
      if (this.bulletCounter <= 0) {
        this.bulletCounter = custom.firerate;
    ...
    if (Math.sqrt(this.vel.x * this.vel.x + this.vel.y * this.vel.y) > custom.maxSpeed) {
      this.vel.x *= custom.acceleration;
      this.vel.y *= custom.acceleration;
    }
  };
...
Bullet = function () {
  ...
  this.bridgesV = false;
  this.postMove = this.wrapPostMove;
  // asteroid can look for bullets so doesn't have to be other way around
